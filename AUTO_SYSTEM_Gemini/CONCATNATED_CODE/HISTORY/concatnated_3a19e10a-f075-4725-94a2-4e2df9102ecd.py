...
class DataPreprocessor:
    ...
    def process(self, df: pd.DataFrame) -> dict:
        ...
        # Chronological split
        train_val_split_index = int(len(X) * (1 - self.test_split_size))
        X_train_val, X_test = X[:train_val_split_index], X[train_val_split_index:]
        y_train_val, y_test = y[:train_val_split_index], y[train_val_split_index:]

        # Adjust validation split size relative to the new training_validation set
        train_split_index = int(len(X_train_val) * (1 - (self.val_split_size / (1 - self.test_split_size))))
        X_train, X_val = X_train_val[:train_split_index], X_train_val[train_split_index:]
        y_train, y_val = y_train_val[:train_split_index], y_train_val[train_split_index:]
