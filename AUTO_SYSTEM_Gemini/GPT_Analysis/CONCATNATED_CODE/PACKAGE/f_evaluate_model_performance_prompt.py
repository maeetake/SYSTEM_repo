...
def main():
    ...
    class DummyKerasModel:
        def predict(self, x: np.ndarray) -> np.ndarray:
            """
            Mimics a Keras model's predict function. Returns the 'Close' price at last timestep plus noise.
            ...
            if x.ndim != 3 or x.shape[2] < 1:
                raise ValueError(f"Input array must be 3D with at least 1 feature, got shape {x.shape}")
            last_close = x[:, -1, -1]  # 'Close' at t-0 (assumed last feature)
            noise = np.random.uniform(-0.01, 0.01, size=(x.shape[0],))
            return last_close + noise  # shape (N,)

    if torch is not None:
        class DummyTorchModel(torch.nn.Module):
            def forward(self, x):
                # Return mean 'Open' over last 3 days + noise
