...
def preprocess_data(
    ...
    assert X.shape[0] == y.shape[0], "Sequence/Input shape mismatch: X and y have different number of samples."
    return X, y, scaler
...
def train_model(
    ...
    assert X_val.shape[0] == y_val.shape[0], "X_val and y_val have different number of samples."
    assert X_train.shape[1:] == X_val.shape[1:], "Train and validation sequence shapes are incompatible."
    assert len(X_train.shape) == 3, "X_train is not 3D."
    assert len(y_train.shape) == 1, "y_train is not 1D."
    assert len(y_val.shape) == 1, "y_val is not 1D."
    ...
    if os.path.exists(checkpoint_path):
        try:
            ...
            logging.warning(f"Could not load checkpoint weights: {e}")
...
def split_data_chronologically(
    ...
    assert len(X_train) + len(X_val) + len(X_test) == n, "Data split mismatch: total does not sum to original."
...
def main():
    ...
    # ---- PATCH: Adapt engineered-column input to expected OHLC columns -------
    if not all(col in df.columns for col in ['Open', 'High', 'Low', 'Close']):
        found = True
        for base in ['Open', 'High', 'Low', 'Close']:
            candidate_col = f"{base}_t-0"
        ...
        if found:
            ...
            logging.warning("Input has engineered columns; mapped *_t-0 columns to OHLC.")
        else:
            logging.error("Input DataFrame is missing both standard OHLC and *_t-0 columns.")
            print(df.head())
            return
