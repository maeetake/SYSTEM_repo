...
def load_data(data_path: str) -> pd.DataFrame:
    ...
    # Check shape and warn if empty
    if df.shape[0] == 0 or df.shape[1] == 0:
        logging.warning(f"Loaded data from {data_path} is empty. Data shape: {df.shape}")
    return df
...
def main():
    ...
    Builds both LSTM and Transformer models and displays their summaries.
    """
    ...
    try:
        ...
        print(df.head())
        # Example: Assume data contains exactly OHLC columns for 1 time step.
        expected_num_features = 4
        # Suppose we want to use last 60 days for prediction:
        sequence_length = 60
        # For a real scenario, you would shape data into (samples, sequence_length, num_features)
        # For this demo, we'll mock input_shape:
        input_shape = (sequence_length, expected_num_features)

        # Optionally, check array shape compatibility if you have the np.ndarray
        # For demonstration, skipping that because data extraction/reshaping is not in this module
        ...
        print(str(e))
        # Optionally provide mock data and proceed to model building with hardcoded input_shape
        mock_data = pd.DataFrame({
            "Open": [0.1, 0.2, 0.3, 0.4],
            "High": [0.2, 0.3, 0.4, 0.5],
            "Low": [0.09, 0.19, 0.29, 0.39],
            "Close": [0.15, 0.25, 0.35, 0.45]
        })
        print("Using mock data:")
        print(mock_data.head())

        # Mock input_shape (for 10 time steps, 4 features as expected OHLC)
        input_shape = (10, 4)
        try:
            lstm_model = build_lstm_model(input_shape)
            transformer_model = build_transformer_model(input_shape)
            print("\nLSTM model summary (using mock input):")
            lstm_model.summary()
            print("\nTransformer model summary (using mock input):")
            transformer_model.summary()
        except Exception as model_exception:
            print(f"Model building failed: {model_exception}")
    except Exception as e:
