...
def train_model(model: tf.keras.Model,
                ...
                X_val: np.ndarray,
                y_val: np.ndarray,
                training_params: Dict[str, Any]) -> Tuple[tf.keras.Model, Dict]:
    """
    Trains a given Keras model on the provided training and validation datasets.

    It uses early stopping to prevent overfitting and restores the best model weights
    based on validation loss.

    Args:
        model (tf.keras.Model): The compiled Keras model to be trained.
        X_train (np.ndarray): Training data features of shape (num_samples, sequence_length, num_features).
        y_train (np.ndarray): Training data targets of shape (num_samples,).
        X_val (np.ndarray): Validation data features of shape (num_samples, sequence_length, num_features).
        y_val (np.ndarray): Validation data targets of shape (num_samples,).
        training_params (Dict[str, Any]): Dictionary of training parameters such as
                                          'epochs', 'batch_size', and 'patience'.

    Returns:
        Tuple[tf.keras.Model, Dict]: A tuple containing the trained model with the best weights
                                    and a dictionary of the training history.
    """
    # Configure callbacks for early stopping and saving the best model
    # The `restore_best_weights=True` argument ensures that the model returned
    # has the weights from the epoch with the lowest validation loss.
    ...
    logger.info(f"--- Finished Training for {model.name} ---")
    
    # The Keras History object itself is returned, which contains the .history dict
