...
class DataPreprocessor:
    ...
    def __init__(self, sequence_length: int = 60, test_split_size: float = 0.1, val_split_size: float = 0.1):
        ...
        Args:
            ...
            test_split_size (float): The proportion of the total sequenced data to reserve for the test set.
            val_split_size (float): The proportion of the total sequenced data to reserve for the validation set.
        """
...
def build_lstm_model(input_shape: tuple) -> keras.Model:
    """
    Builds and compiles an LSTM model based on the provided input shape using the Functional API.
    ...
    x = layers.LSTM(50, return_sequences=True, name='lstm_1')(inputs)
    x = layers.Dropout(0.2, name='dropout_1')(x)
    x = layers.LSTM(50, return_sequences=False, name='lstm_2')(x)
...
class PositionalEmbedding(layers.Layer):
    """
    Custom Keras layer to add learnable positional embeddings.
    """
...
def build_transformer_model(input_shape, head_size=256, num_heads=4, ff_dim=256, num_transformer_blocks=4, mlp_units=[128], dropout=0.1, mlp_dropout=0.1):
    """
    ...
    Args:
        ...
        ff_dim (int): The dimension of the feed-forward network. A reasonable default is same as head_size.
    ...
    # Project input features to the head_size dimension
    x = layers.Dense(head_size)(x)
    
    # Add positional encoding
    x = PositionalEmbedding(sequence_length=input_shape[0], embed_dim=head_size, name="positional_embedding")(x)
    ...
    optimizer = keras.optimizers.Adam(learning_rate=1e-4) # Use a smaller learning rate for stability
...
def main():
    ...
    try:
        ...
        print(f"\n[ERROR] Could not load data from '{data_path}'.")
        print("Please ensure the file exists in the same directory as the script, or provide a full, correct path.")
        print("---")
        print("Proceeding with mock data for demonstration purposes only.")
        # Generate a proper chronological time series for the mock data
        print("Generating a valid chronological mock dataset...")
        total_days = 300
        mock_data = {
            'Date': pd.date_range(start='2020-01-01', periods=total_days, freq='D'),
            'Open': np.linspace(150, 250, total_days) + np.random.randn(total_days) * 2,
            'High': np.linspace(152, 255, total_days) + np.random.randn(total_days) * 2,
            'Low': np.linspace(148, 248, total_days) + np.random.randn(total_days) * 2,
            'Close': np.linspace(151, 252, total_days) + np.random.randn(total_days) * 2,
        }
        ...
        try:
            ...
            for key, value in final_datasets.items():
                if isinstance(value, np.ndarray):
                    # Check for non-empty splits
                    if value.shape[0] == 0:
                         raise ValueError(f"Data split '{key}' is empty. Check split ratios or increase dataset size.")
