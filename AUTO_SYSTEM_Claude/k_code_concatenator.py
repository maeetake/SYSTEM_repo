...
from config import (
    OUT_JSON_DIR,
    CONCATNATED_CODE_DIR,
    PACKAGE_DIR,
    HISTORY_DIR,
    DATA_MAKER_INIT_DIR,
    NO_ERROR_CODE_DIR,
    GEMINI_MODEL_NAME as MODEL_NAME
)
...
def copy_directory():
    ...
    src = NO_ERROR_CODE_DIR
    dst_list = [
        PACKAGE_DIR,
        os.path.join(DATA_MAKER_INIT_DIR, "PACKAGE"),
        os.path.join(HISTORY_DIR, "PACKAGE")
    ]
...
def main():
    ...
    if len(packages) == 1:
        while True:
            print("enter_1")
            package_path = os.path.join(PACKAGE_DIR, packages[0])
            code = generate_first_concatnated_code(package_path) 
            generated_code_path = save_generated_code(code, CONCATNATED_CODE_DIR, filename="concatnated_1.py")
        ...
        model_specification_path = os.path.join(OUT_JSON_DIR, "3.model_specification.json")
        max_filename, next_filename = find_max_script_number(CONCATNATED_CODE_DIR)
        concatnated_code_path = os.path.join(CONCATNATED_CODE_DIR, max_filename)
        additional_code_path = os.path.join(PACKAGE_DIR, get_latest_alphabet_script(PACKAGE_DIR))
        ...
        generated_code_path = save_generated_code(code, HISTORY_DIR, history_filename)
        ...
        while True:
            if llm_check.strip() != "Error":
                ...
                python_modules_path = PACKAGE_DIR
                ...
                issues = evaluation_json["evaluation"]["issues"]
                ...
                if issues:
                    ...
                    while True:
                        unique_id = str(uuid.uuid4())
                        history_filename = f"concatnated_{unique_id}.py"
                        generated_code_path = save_generated_code(re_concatted_code, HISTORY_DIR, history_filename)
                        result, errors = execute_generated_code(generated_code_path)
                        print("===RESULT===")
                        print(result)
                        print("===ERRORS===")
                    ...
                    if count == 3:
                        ...
                        save_generated_code(code, CONCATNATED_CODE_DIR, next_filename)
                ...
                specifications = (
                    "### Instructions:\n"
                    "1. When making corrections, consider the original purpose and intent of the code to ensure it works as intended.\n"
                    "2. Pay attention to the input and output formats of the modules being used. If necessary, implement functionality to transform or preprocess the data so that it conforms to the expected formats.\n"
                    "3. Ensure that all data processing, model training, and evaluation steps follow logical consistency and provide meaningful results.\n"
                )
